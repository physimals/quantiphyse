...
class Picker(LogSource):
    ...
    def pick(self, win, pos):
        ...
        :param win: Index ID of the ortho viewer window sending the event
...
class PaintPicker(Picker):
    ...
    def pick(self, win, pos):
        self.win = win
        self.active_view = self.ivl.ortho_views[self.win]
        self._points = [pos,]
...
class PolygonPicker(Picker):
    ...
    def pick(self, win, pos):
        if self.win is None:
            ...
            self.active_view = self.ivl.ortho_views[self.win]
            self.active_view.add(self.roisel)
        elif win != self.win:
            return
    ...
    def cleanup(self):
        if self.active_view is not None:
            self.active_view.remove(self.roisel)

    def _xy_coords(self, pos):
        return float(pos[self.active_view.xaxis]), float(pos[self.active_view.yaxis])

    def _grid_points(self, grid):
        grid_axes = grid.get_ras_axes()
        gridx = grid_axes[self.active_view.xaxis]
        gridy = grid_axes[self.active_view.yaxis]
        gridz = grid_axes[self.active_view.zaxis]
        ...
        for x, y in self._points:
            std_pt = [0, 0, 0, 0]
            std_pt[self.active_view.xaxis] = x
            std_pt[self.active_view.yaxis] = y
            std_pt[self.active_view.zaxis] = 0
...
class FreehandPicker(PolygonPicker):
    ...
    def pick(self, win, pos):
        ...
        fx, fy = float(pos[self.active_view.xaxis]), float(pos[self.active_view.yaxis])
